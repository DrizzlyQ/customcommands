...
from pymongo import MongoClient # open cmd > python -m pip install pymongo
...
token = os.environ.get("BOT_TOKEN") #create .env file > open with notepad > Type the following as said. Example: BOT_TOKEN="TOKEN"
...
async def add_cmd(message):
	...
	# checking that it's at least 3 words long (roughly in the form of !add <command> <paste>)
	if len(tmpArr) > 2:
		command = tmpArr[1]
		prefix = "!add " + command
